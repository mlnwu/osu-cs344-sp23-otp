...
// Validates characters ciphertext and key files
void validate_chars(char *ciphertext, char *key) {
...
// Creates message with ciphertext and key 
void create_msg(char *buffer, const char *ciphertext, const char *key) {
  FILE *fp;
  char line[MAX];
...
int main(int argc, char *argv[]){
  ...
  // Get file lengths
  int cipher_text = open(argv[1], O_RDONLY);
  int cipher_length = lseek(cipher_text, 0 , SEEK_END);
  int key_text = open(argv[2], O_RDONLY);
  int key_length = lseek(key_text, 0, SEEK_END);

  // Check if key is too short
  if(cipher_length > key_length){
  ...
  // Check if message is from dec
  char *from = strtok(buffer, "\n");
  if(from[0] == 'B'){
    error("Error: Message not from dec\n");
  }
